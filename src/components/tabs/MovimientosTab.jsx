...
export default function MovimientosTab({
  ...
  incomeCategories, categories, selMonth, setTab, userNames, USERS,
}) {
  const { C, Sx } = useTheme();
  ...
  const shown = allMovements.filter(m =>
    userFilter === 'todos' || m.user === userFilter
  );
  ...
  const closeEditor = () => { setSelectedId(null); setEditForm(null); };
    ...
    <div>
      ...
      <div style={{ background: C.coral, borderRadius: 14, padding: '18px 16px 20px', marginBottom: 14 }}>
        <div style={{ marginBottom: 14 }}>
          ...
          <div style={{ fontSize: 13, color: 'rgba(255,255,255,0.65)' }}>{MONTHS_FULL[selMonth]} · {shown.length} registros</div>
        </div>
      ...
      {/* Filtros chips por usuario */}
      <div style={{ display: 'flex', gap: 8, marginBottom: 14, flexWrap: 'wrap' }}>
        <button onClick={() => setUserFilter('todos')} style={chipStyle(userFilter === 'todos', C.coral, C.coralL)}>
          Todos
        </button>
        {userNames.map(name => (
          <button key={name} onClick={() => setUserFilter(name)} style={chipStyle(userFilter === name, C.sage, C.sageL)}>
            {name}
          </button>
        ))}
      </div>
            ...
            {isSelected && editForm && (
              <div style={{
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
                  ...
                  <button onClick={closeEditor} style={{ ...Sx.xbtn, fontSize: 18 }}>✕</button>
                </div>
                ...
                {editForm._type === 'egreso' ? (
                  ...
                  <div style={{ marginBottom: 10 }}>
                    ...
                    <select
                      ...
                      onChange={e => setEditForm({ ...editForm, incomeCategory: e.target.value })}
                      style={{ ...Sx.inp, cursor: 'pointer' }}
                    >
                ...
                {/* Fecha — date picker unificado para egresos e ingresos */}
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                  <button
                    ...
                    style={{ flex: 1, padding: '10px 0', borderRadius: 8, border: 'none', background: C.peachL, color: C.rose, fontSize: 13, fontWeight: 500, cursor: 'pointer' }}
                  >
                    Eliminar
                  </button>
                  <button
                    onClick={saveEdit}
                    style={{ flex: 2, padding: '10px 0', borderRadius: 8, border: 'none', background: C.coral, color: '#fff', fontSize: 13, fontWeight: 500, cursor: 'pointer' }}
                  >
                    Guardar
                  </button>
                </div>
